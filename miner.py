...
def findnonce(block, init, diff):
    nonce = init
    while(True):
        hash512 = SHA512.new()
        block['nonce'] = str(hex(nonce))
        ...
        if int(hash512.hexdigest(), 16) < 2 ** (492 - diff):
...
def findnonce_np(block, init, diff):
    nonce = init
    while(True):
        hash512 = SHA512.new()
        block['nonce'] = str(hex(nonce))
        ...
        if int(hash512.hexdigest(), 16) < 2 ** (492 - diff):
            # print(hash512.hexdigest())
            my_block = {'hash': hash512.hexdigest(), 'type': 'block_hash', 'block': json.dumps(block)}
            r = requests.post('https://gw.kaist.ac.kr/broadcast.post', json = myblock)
            print('done!!\n\n')
            return True
        nonce += diff
...
if __name__ == '__main__':
    t1 = threading.Thread(target=findnonce, args=(block, 0, 2))
    t2 = threading.Thread(target=findnonce_np, args=(block, 1, 2))
