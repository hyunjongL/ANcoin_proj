...
# genPublicKey
# input  : modulus, publicExponent as string
# output : RSA Public Key
def genPublicKey(mod, pub):
    payload = (int(mod, 16), int(pub, 16))
    return RSA.construct(payload)
...
def verify(pk, msg, sig, hash_512):
    hash_512.update(str.encode(msg))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        ...
        if sys.argv[1] == '-s':
            ...
            txt = input()
            # sign the message
            signature = sign(genPrivateKey(payload['modulus'],
                                           payload['publicExponent'],
                                           payload['privateExponent']),
                             txt, hash_512)
        ...
        # option -p : make a payload with given input.
            ...
            if not(check_dict_keys(payload, ('message', 'sig'))):
                print('Wrong input file: The json file has wrong arguments')
                quit()
            elif not(check_dict_keys(payload['sig'],
                     ('modulus', 'publicExponent', 'signature'))):
                print('Wrong input file: Sig in jsonfile has wrong arguments')
                quit()

            f = open('verify', 'w')
            if verify(genPublicKey(payload['sig']['modulus'],
                                   payload['sig']['publicExponent']),
                      payload['message'],
