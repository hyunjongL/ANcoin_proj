...
def do_test(input_text, key_json, payload_correct_json, payload_wrong_json):
    ...
    counter = 0
    for i in range(10):
        random_string = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(27))
        with open('temporary_text_file_for.test', 'w') as openfile:
            openfile.write(random_string)
            openfile.close()
        os.system("python3 sig.py -p " + key_json + " < temporary_text_file_for.test")
        os.system("python3 sig.py -v payload.json")
        with open('verify') as verify:
            l = verify.readline()
...
if len(sys.argv) != 1:
    ...
    do_test(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
else:
    ...
    do_test('input.txt', 'examples/key.json', 'examples/payload.correct.json', 'examples/payload.wrong.json')
